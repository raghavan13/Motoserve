...
//  Motoserve
//
//  Created by Karthik Baskaran on 26/11/18.
...
- (void)createDesign
{
    ...
    userNameTxt.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"New Password" attributes:@{NSForegroundColorAttributeName: color}];
    ...
    UIButton * signupBtn=[[UIButton alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/2.0-75, CGRectGetMaxY(UserLine.frame)+40, 150, 30)];
    [signupBtn setBackgroundColor:Singlecolor(whiteColor)];
    [signupBtn setTitle:@"Update Password" forState:UIControlStateNormal];
...
- (void)signupAction
{
    if ([self isValidEntry])
    {
        ...
        NSString *url =[UrlGenerator PostResetpass];
        AFHTTPSessionManager *manager = [[AFHTTPSessionManager alloc]initWithSessionConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
        manager.requestSerializer = [AFJSONRequestSerializer serializer];
        [manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
        NSDictionary * parameters = @{
                                      @"_id":self.userid,@"password":userNameTxt.text
                                      };
